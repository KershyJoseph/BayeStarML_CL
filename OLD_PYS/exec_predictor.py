...
from BayestarML.preprocess import prepare_pred4
from BayestarML.utils import model_pred_plotter

from BayestarML.predict import predict4


def bart_bhs_pred(target: str):
    """Train BART and BHS and then make some predictions"""
    X, X_er = prepare_pred4("Datasets/estrellas_anfitrionas.txt")
...
def bart_bhs_train(target):
    ...
    """
    _, bhs_pred, bhs_w, X, Xer, y = predict4(
        X=None,
        X_er=None,
        target=target,
        training_dataset_path="DataExploring/good_MS.txt",
        ...
        NNnodes=16,
        test=True,
    )  # disregards X, X_er for test=True / uses test values

    target_ms = bhs_pred.mean(0)
    target_stds = bhs_pred.std(0)

    model_pred_plotter(
        y, target_ms, target_stds, target, "BHS", "outputs700ms/BHSmass", "train"
    )
    ...
    plt.plot(X["logL"], y, "x", label="True " + target)
    ...
    plt.plot(X["logg"], y, "x", label="True " + target)
