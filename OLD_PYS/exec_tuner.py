...
from pred_sampling import sample_post_pred_HBNN_para
from preprocess import denormalise_val, get_dataset, return_train_test
from utils import mard, train


def objective(
    trial, data, min, max, score: str, draw=1000, chains=4, target_accept=0.95
):
    nodes = trial.suggest_int("nodes", min, max)

    model = hbnn.HBNN_M4(
        data.x_train, data.mass_train, data.x_train_er, data.emass_train, nodes
    )
    trace = train(
        model,
        draw=draw,
    ...
    if score == "elpd_loo":
        ...
        pred, lpd = sample_post_pred_HBNN_para(
            trace, data.x_test, data.x_test_err, nodes, 4, "Mass"
        )
        means = pred.mean(0)
...
if __name__ == "__main__":
    ...
    study.optimize(
        lambda trial: objective(trial, dataset, min, max, score=score), n_trials=n
    )
    ...
    plt.figure()
    plt.plot(
        df_results["number"], df_results["value"], "bo", label=score + "at trial n"
    )
    running_bests = best_scores_at_trial_n(score="MARD", all_scores=df_results["value"])
    plt.plot(
        df_results["number"], running_bests, "b-", label="Best " + score + " at trial n"
    )
