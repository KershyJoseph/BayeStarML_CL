...
def HBNN_M4(X_train, Y, X_error, Y_error, n_hidden):
    ...
    with pm.Model() as neural_network:
        ...
        # Weights from input to hidden layer
        weights_in_1 = pm.Normal(
            "w_in_1", 0, sigma=0.35*np.sqrt(2/4), shape=(n_hidden, 4) #replaced X_latent.eval().shape[1]
        )
        ...
        # Weights from hidden layer to output
        weights_2_out = pm.Normal("w_2_out", 0, sigma=0.45*np.sqrt(2/n_hidden), shape=n_hidden)

        bias_1 = pm.Normal("bias_1", 0, sigma=0.035, shape=n_hidden)

        bias_2 = pm.Normal("bias_2", 0, sigma=0.045, shape=n_hidden)
