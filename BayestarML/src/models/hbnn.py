...
def HBNN_M4(X_train, Y, X_error, Y_error, n_hidden):
    ...
    with pm.Model() as neural_network:
        ...
        weights_1_2 = pm.Normal(
            "w_1_2", 0, sigma=0.05, shape=(n_hidden,n_hidden)
        ) #.12

        # Weights from hidden layer to output
        weights_2_out = pm.Normal("w_2_out", 0, sigma=0.1, shape=n_hidden) #.3
...
def HBNN_M4_simpler(X_train, Y, X_error, Y_error, n_hidden=5):
    ...
    ***Edit to only have one layer of 5 nodes***
    ...
    with pm.Model() as neural_network:
        ...
        # Weights from input to hidden layer
        weights_in_1 = pm.Normal(
            "w_in_1", 0, sigma=np.sqrt(2/4)*0.05, shape=(n_hidden, 4) #X_latent.eval().shape[1])
        )
